...
> **Status:** This document defines the TACO security model. Implementation of these patterns is optional for sandbox deployments and recommended for production multi-company deployments.
...
TACO agents declare authentication using standard A2A `securitySchemes` and `security` fields in the Agent Card. TACO does not add new scheme types — it recommends which of A2A's five schemes to use per deployment scenario.
...
A TACO agent that requires authentication declares it in the standard A2A `securitySchemes` and `security` fields, alongside the `x-construction` extension:
...
```json
{
      ...
      "flows": {
        "clientCredentials": {
          ...
          "scopes": {
            ...
            "taco:task:value-engineering": "Submit value engineering tasks"
          }
        }
      }
    }
  ...
  "x-construction": {
    ...
    "security": {
      "trustTier": 1,
      "scopesOffered": ["taco:trade:mechanical", "taco:task:estimate", "taco:task:value-engineering"],
      "projectScoped": true,
...
The `securitySchemes` key name (`tacoOAuth`) is locally defined. TACO recommends using a descriptive name that includes the scheme type.

## TACO Scope Taxonomy

TACO defines a scope naming convention that makes construction authorization semantics machine-readable. These scope names are declared in `securitySchemes`, requested by orchestrators during token acquisition, and validated by agents on incoming requests.
...
|---------------|---------|---------|
| `taco:trade:{trade}` | Access to all workflows for a given trade | `taco:trade:mechanical` |
| `taco:task:{taskType}` | Permission to submit a specific task type | `taco:task:estimate` |
| `taco:csi:{division}` | Access scoped to a CSI MasterFormat division | `taco:csi:23` |
| `taco:project:{id}:read` | Read access to a specific project's artifacts | `taco:project:PRJ-0042:read` |
| `taco:project:{id}:write` | Write (task submission) access on a specific project | `taco:project:PRJ-0042:write` |
| `taco:registry:read` | Discover agents in the TACO Registry | `taco:registry:read` |
| `taco:registry:publish` | Publish or update an Agent Card in the Registry | `taco:registry:publish` |

The `{trade}` values match the `trade` field in [`agent-card-extensions.md`](agent-card-extensions.md). The `{taskType}` values match the task types defined in [`task-types.md`](task-types.md). The `{id}` in project scopes should match the `projectId` field used in TACO data schemas (`bom-v1`, `rfi-v1`, etc.).

### Scope Combination Rules

1. **Scopes are additive.** An orchestrator requesting both `taco:trade:mechanical` and `taco:task:estimate` can only call estimate tasks on mechanical agents.
2. **Project scopes require a task or trade scope.** A `taco:project:{id}` scope is not meaningful alone — it gates which project's data is accessible, while trade and task scopes gate what actions are permitted.
3. **Agents validate scope against task type.** An agent advertising `taco:task:takeoff` should reject tokens that only carry `taco:task:estimate`. The incoming token's scopes must include the task type being requested.
...
1. GC orchestrator authenticates to the auth server with `client_credentials`; receives a token scoped to `taco:trade:mechanical taco:project:PRJ-0042:write`.
2. Before calling the mechanical estimating agent, the orchestrator exchanges its token for one scoped to `taco:task:estimate taco:project:PRJ-0042:write` (narrowed: no longer full trade scope, only the estimate task).
3. Estimating agent validates the narrow token; executes the estimate; returns `estimate-v1`.
4. If the estimating agent needs to call the supplier quote agent, it exchanges again: requests `taco:task:material-procurement taco:project:PRJ-0042:write`.
...
Agents must not forward their own received token to downstream agents. This prohibition mirrors the MCP specification's stance on user tokens. In TACO's multi-company context, it also prevents a compromised agent from escalating its authority by re-using its caller's token with a different downstream agent. Always perform a Token Exchange to obtain a new, appropriately-scoped token for each hop.

## Registry Trust Model

The TACO Agent Registry is the mechanism by which orchestrators discover agents. Trust in a discovered agent depends on how it was listed.

### Trust Tiers

| Tier | Label | How Achieved | What It Means |
|------|-------|-------------|---------------|
| 0 | Unverified | Self-published via `taco:registry:publish` scope | Agent Card is indexed; no claims are validated. Suitable for development and sandbox use. |
...
A2A supports signing Agent Cards using JSON Web Signature ([RFC 7515](https://datatracker.ietf.org/doc/html/rfc7515)). TACO recommends that Tier 1 and Tier 2 agents sign their cards to prevent tampering when cards pass through intermediary registries or brokers. A signed card proves the card content hasn't been modified in transit — it does not by itself establish trust tier.

### Registry Queries and Auth

Querying the registry to discover agents requires a `taco:registry:read` scope. The registry returns Agent Cards in full. The consuming orchestrator is responsible for evaluating the trust tier and deciding whether to proceed. Agents at higher trust tiers may appear higher in search results, but the registry does not filter out lower-tier agents unless the query explicitly requests a minimum tier.

## x-construction Security Extension

TACO adds a `security` sub-object to the top-level `x-construction` extension to carry construction-specific security metadata that supplements A2A's `securitySchemes`. See [`agent-card-extensions.md`](agent-card-extensions.md) for the full extension specification.
...
| `scopesOffered` | string[] | No | TACO scopes this agent will accept on incoming requests. Derived from the Agent Card's `securitySchemes`; surfaced here for quick registry filtering without requiring a full OAuth discovery flow. |
| `projectScoped` | boolean | No | If `true`, the agent requires a `taco:project:{id}` scope in addition to task/trade scopes. Allows orchestrators to know in advance whether they need a project-specific token. |
...
Trade scopes (`taco:trade:{trade}`) align with how construction projects are organized. A mechanical subcontractor's agent should only accept tasks for mechanical trade workflows. A multi-trade GC orchestrator may hold multiple trade scopes but should only pass the relevant trade scope to each downstream agent via Token Exchange.

### Project Membership

In a typical commercial construction project, three levels of project access exist:

1. **Project owner / GC** — full project write access across trades (`taco:project:{id}:write` + multiple `taco:trade:*` scopes).
2. **Trade contractors** — write access to their own CSI divisions within the project (`taco:project:{id}:write` + `taco:csi:{div}` + `taco:trade:{trade}`).
3. **Consultants** (architects, engineers) — read access for review tasks (`taco:project:{id}:read`).

The TACO scope taxonomy can express all three levels through scope combination.
...
3. Define TACO scopes for each task type and trade the agent supports; add them to `x-construction.security.scopesOffered`.
...
6. If the agent is project-scoped, extract `projectId` from the incoming data payload and verify the token carries a matching `taco:project:{id}` scope.
7. Submit to the TACO Registry for trust tier elevation after achieving SOC2, ISO27001, or equivalent certification.

## Adding New Scopes

New scope dimensions or patterns can be proposed via GitHub issue. A proposal should include:

1. Scope string following the `taco:{dimension}:{value}` pattern
